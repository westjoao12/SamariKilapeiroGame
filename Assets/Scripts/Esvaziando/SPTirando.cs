...
public class SPTirando : MonoBehaviour
{
    ...
    //public GameObject ColisorDescendoPri;
    string MissaoActual;
    ...
    private void OnCollisionEnter(Collision jogador)
    {
        if (jogador.gameObject.CompareTag("Player"))
        {
            ...
            {
                Seguranca.gameObject.SetActive(false);
                SegurancaPri.gameObject.SetActive(true);
            }
            ...
            {
                ComputadorMesa[i].gameObject.SetActive(false);
            }


            for (int i = 0; i < CarteirasPri.Length; i++)
            {
                CarteirasPri[i].gameObject.SetActive(true);
            }


            for (int i = 0; i < SanitasPri.Length; i++)
            {
                SanitasPri[i].gameObject.SetActive(true);
            }
